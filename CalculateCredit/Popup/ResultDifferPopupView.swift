...
class ResultDifferPopupView: UIView {
    
    var rootVC: UIViewController!
    static func createPopup(months: Int, percent: CGFloat, sum: Int, rootVC: UIViewController) -> ResultDifferPopupView {
        print("months: \(months)\npersent: \(percent)\nsum: \(sum)")
        let popup = Bundle.main.loadNibNamed("DifferPopupView", owner: self, options: nil)![0] as! ResultDifferPopupView
    ...
    fileprivate func openSlider() {
        ...
        let payment = HandlerCalculator.shared.solveDifferByMonth(sum: sum, percent: percent, months: months, currentMonth: 0)
    ...
    fileprivate func closeSlider() {
        ...
        let firstPayment = HandlerCalculator.shared.solveDifferByMonth(sum: sum, percent: percent, months: months, currentMonth: 1)
        let lastPayment = HandlerCalculator.shared.solveDifferByMonth(sum: sum, percent: percent, months: months, currentMonth: months)
    ...
    func fillPopupData() {
        ...
        let allPayments = HandlerCalculator.shared.solveGeneralSumByMonths(sum: sum, percent: percent, months: months)
    ...
    @objc fileprivate func sliderChanged() {
        ...
        if selectedMonth != month {
            selectedMonth = month
            let payment = HandlerCalculator.shared.solveDifferByMonth(sum: sum, percent: percent, months: months, currentMonth: month)
            
            DispatchQueue.main.asyncAfter(deadline: .now()+0.2, execute: {
                self.paymentButton.setTitle("\(Int(payment))".stringByNumberWithSeparators, for: .normal)
            })
        }
        monthOfPaymentLabel.text = "pago por  \(month) mes)"
