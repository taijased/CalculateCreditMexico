...
class ResultAnnuitPopupView: UIView {

    var rootVC: UIViewController!
    
    static func createPopup(months: Int, percent: CGFloat, sum: Int, rootVC: UIViewController) -> ResultAnnuitPopupView {
        print("months: \(months)\npersent: \(percent)\nsum: \(sum)")
        let popup = Bundle.main.loadNibNamed("AnnuitPopupView", owner: self, options: nil)![0] as! ResultAnnuitPopupView
    ...
    func fillPopupData() {
        let payment = HandlerCalculator.shared.solveAnnuitet(sum: sum, percent: percent, months: months)
